class GPUBigNum {
  constructor(limbCount = 64) {
    this.L = limbCount; // 64 limbs * 32-bit = 2048-bit precision
    this.device = null;
    this.pipelines = {};
  }
  ...
  async init() {
    ...
    const shaderCode = `
      ...
      // Returns true if local_A >= global_B
      fn gte(a: ptr<function, array<u32, L>>, b_idx: u32) -> bool {
        for (var k = 0u; k < L; k++) {
          let i = L - 1u - k; // High to Low
          let val_a = (*a)[i];
      ...
      fn main_mul(@builtin(global_invocation_id) id : vec3<u32>) {
        ...
        for (var i = 0u; i < L; i++) {
          ...
          for (var j = 0u; j < L; j++) {
            let res = mac(a_val, bufB.values[idx].limbs[j], bufR.values[idx].limbs[i+j], carry);
      ...
      fn main_div(@builtin(global_invocation_id) id : vec3<u32>) {
        let idx = id.x;
        if (idx >= arrayLength(&bufA.values)) { return; }
        var rem : array<u32, L>; var quo : array<u32, L>;
        ...
        for(var i=0u; i<L; i++) { bufR.values[idx].limbs[i] = quo[i]; } // Write Quotient
        for(var i=L; i<L*2; i++) { bufR.values[idx].limbs[i] = 0u; }
      }
      ...
      fn main_mod(@builtin(global_invocation_id) id : vec3<u32>) {
        ...
        // Only need to track Remainder
        var rem : array<u32, L>;
        for(var i=0u; i<L; i++) { rem[i] = 0u; }

        for (var k = 0u; k < L; k++) {
           let limb_idx = L - 1u - k;
           let limb_val = bufA.values[idx].limbs[limb_idx];
           for (var b = 0u; b < 32u; b++) {
             // Shift left, pull in MSB from A
             shl1(&rem, (limb_val >> (31u - b)) & 1u);
             
             // If Rem >= B, subtract B
             if (gte(&rem, idx)) {
               sub_self(&rem, idx);
               // No need to track quotient bit
             }
           }
        }
        for(var i=0u; i<L; i++) { bufR.values[idx].limbs[i] = rem[i]; } // Write Remainder
        for(var i=L; i<L*2; i++) { bufR.values[idx].limbs[i] = 0u; }
      }
    `;
  ...
  async mod(listA, listB) { return this._runOp(listA, listB, 'mod'); }
